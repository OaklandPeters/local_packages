...
(function(Meta,undefined){
	...
	Meta.has = function(obj,properties){
		...
		//properties = ensure_array(properties);
	...
	Meta.ensure_array = function(obj){
		console.error("This function is deprecated. Use Meta.ensureArray instead.");
		return Meta.ensureArray(obj);
	}
...
//		Meta.assert(Meta.isKlass(obj,klasses),message);
		if (! Meta.isKlass(obj,klasses)) {
			message = message || "Invalid object. Should be one of: '"+String(klasses)+"'";
			throw Meta.error(message)
		}
	}
