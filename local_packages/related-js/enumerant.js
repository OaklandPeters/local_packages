...
//=========== 
//=========== REMAINING TASK: mixin with underscore


var depth = ['0','1','2'];
var results = _(depth).chain()
	.map(type.int.to)
	.enumerate([0,70],[1,50],[2,30])
	.value();

//Improve the way that Cases are displayed --> perhaps add a .display() property
// Ex. so [type.int,type.str] --> "Integer (type.int), String (type.str)"
...
_.mixin({
	enumerate: function() {
		...
		var caseArray = args.slice(1);
		
		var cases = _(caseArray).map(function(caseArg){
			//Build Case Objects from in array: [values,returns]
			console.log("caseArgs: ",caseArgs);
			if (caseArgs.length==1) {
				return Case.default(caseArgs);
			} else {
				return Case.basic(caseArgs[0],caseArgs[1]);
			}
		});
...
//============ Sample syntax
//--- Wrapped Case Functions
//If no arguments in enumerate()--> return wrapped function. 
//Wrapped returns value after .defaults() or .value() 
_(depth).enumerate()	
	.case(0,70)
	.case(1,50)
	.case(2,30)
	.defaults(20);
//--- Non-wrapped case functions
_(depth).enumerate(function(val){
	this.case(0,70)
		.case(1,50)
		.case(2,30)
		.case
});
_(depth).enumerate([(0,70),(1,50),(2,30),20])							
//Add
