...
from .. import rich_core #pylint: disable=relative-import
...
# class QualifiedName(object):
    def __new__(cls, database=None, table=None, column=None, alias=None):
        database, table, column, alias = cls.validate(database, table, column, alias)
        return cls.combine(database, table, column, alias)
        
    @classmethod
    def validate(cls, database=None, table=None, column=None, alias=None):
        #[] Validation - for nullable strings
        nullable = (type(None), basestring)
        rich_core.AssertKlass(database, nullable)
        rich_core.AssertKlass(table, nullable)
        ...
        return database, table, column, alias
    @classmethod
    def combine(cls, database, table, column, alias):
        qual_name = '.'.join([elm for elm
            in [database, table, column]
            if elm != None
        ])
        if alias != None:
            return qual_name + " as " + alias
        else:
            return qual_name
